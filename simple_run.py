...
# load pipelines
device = "cuda"
neuraltexture_controlnet = NeuralTextureControlNetModel.from_pretrained(
    "DiLightNet/DiLightNet"
)
pipe = StableDiffusionControlNetPipeline.from_pretrained(
    "sd2-community/stable-diffusion-2",
    controlnet=neuraltexture_controlnet,
).to(device)

# load cond images
cond_img_path = "examples/input/futuristic_soldier"
mask = imageio.v3.imread(cond_img_path + "_mask.png")[..., :1] / 255.0
source_image = imageio.v3.imread(cond_img_path + ".png")[..., :3] / 255.0
hint_types = ["diffuse", "ggx0.05", "ggx0.13", "ggx0.34"]
images = [mask, source_image]
for hint_type in hint_types:
    image_path = f"{cond_img_path}_{hint_type}.png"
    image = imageio.v3.imread(image_path) / 255.0
...
# run pipeline
image = pipe(
    "futuristic soldier with advanced armor weaponry and helmet", image=hint
).images[0]
