...
class Args:
    ...
    out_vid: Optional[str] = (
        None  # Path to the output video, defaults to the input image path
    )
    ...
    pl_pos_r: float = 5.0  # Rotation radius of the point light
    pl_pos_h: float = 3.0  # Height of the point light
    ...
    env_map: Optional[str] = (
        None  # Environment map for the rendering, defaults to None (white point light)
    )
...
if __name__ == "__main__":
    ...
    if args.radiance_hints_path is not None:
        ...
        pls = [
            (
                args.pl_pos_r * np.sin(frame / frames * np.pi * 2.0),
                args.pl_pos_r * np.cos(frame / frames * np.pi * 2.0),
                args.pl_pos_h,
            )
        ...
        if args.cache_radiance_hints:
            ...
            for i in range(frames):
                if not (
                    os.path.exists(f"{output_folder}/hint{i:02d}_diffuse.png")
        ...
        if render_radiance_hints or render_env_bg:
            ...
            mesh, fov = mesh_reconstruction(
                input_image, mask, False, fov, args.mask_threshold
            )
    ...
    mask_for_bg = imageio.v3.imread(res_path + "/hint00_diffuse.png")[..., -1:] / 255.0
    masked_image = (
        (input_image.astype(np.float32) * mask_for_bg).clip(0, 255).astype(np.uint8)
    )
    relighting_gen(
    ...
    for idx in range(args.num_imgs_per_prompt):
        ...
        for frame in range(frames):
            relit_img = imageio.v3.imread(
                res_path + f"/relighting{frame:02d}_{idx}.png"
            )
            if use_env_map:
                bg = imageio.v3.imread(res_path + f"/bg{frame:02d}.png") / 255.0
