...
pipe = StableDiffusionControlNetPipeline.from_pretrained(
    "sd2-community/stable-diffusion-2", controlnet=controlnet, torch_dtype=dtype
).to(device)
pipe.scheduler = UniPCMultistepScheduler.from_config(pipe.scheduler.config)
pipe.set_progress_bar_config(disable=True)

inpainting_pipe = StableDiffusionInpaintPipeline.from_pretrained(
    "sd2-community/stable-diffusion-2-inpainting", torch_dtype=dtype
).to(device)
...
def relighting_gen(
    ...
    for i in tqdm(range(frames)):
        source_image = masked_ref_img[..., :3] / 255.0
        ...
        for idx in range(num_imgs_per_prompt):
            imageio.imwrite(
                f"{cond_path}/relighting{i:02d}_{idx}.png",
                (images[idx] * 255).clip(0, 255).astype(np.uint8),
            )
