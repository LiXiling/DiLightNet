...
class Options:
    ...
    env_map_list_json: str = (
        "./assets/hdri/polyhaven_hdris.json"  # Path to env map list
    )
    env_map_dir_path: str = "./assets/hdri/files"  # Path to env map directory
    white_env_map_dir_path: str = (
        "./assets/hdri/file_bw"  # Path to white env map directory
    )
...
def render_core(args: Options):
    ...
    from bpy_helper.material import (
        create_white_diffuse_material,
        create_specular_ggx_material,
    ...
    def render_rgb_and_hint(output_path):
        ...
        bpy.context.scene.render.image_settings.file_format = (
            "PNG"  # set output to png (with tonemapping)
        )
        bpy.context.scene.render.filepath = f"{output_path}.png"
        bpy.ops.render.render(animation=False, write_still=True)
        img = imageio.v3.imread(f"{output_path}.png") / 255.0
        if img.shape[-1] == 4:
            img = img[..., :3] * img[..., 3:]  # fix edge aliasing
        imageio.v3.imwrite(
            f"{output_path}.png", (img * 255).clip(0, 255).astype(np.uint8)
        )
        ...
        for mat_name, mat in MAT_DICT.items():
            ...
            bpy.ops.render.render(animation=False, write_still=True)
            img = imageio.v3.imread(f"{output_path}{mat_name}.png") / 255.0
            if img.shape[-1] == 4:
                img = img[..., :3] * img[..., 3:]  # fix edge aliasing
            imageio.v3.imwrite(
                f"{output_path}{mat_name}.png",
    ...
    json.dump(
        {"scale": scale, "offset": array2list(offset)},
    ...
    for eye_idx in range(args.num_views):
        ...
        radius = random.uniform(0.8, 1.1) * (
            0.5 / math.tanh(fov / 2.0 * (math.pi / 180.0))
        )
        ...
        # save cam info
        json.dump(
            {"c2w": mat2list(c2w), "fov": fov},
            open(f"{view_path}/cam.json", "w"),
            indent=4,
        )
        ...
        for white_pl_idx in range(args.num_white_pls):
            ...
            json.dump(
                ...
                open(f"{ref_pl_path}/white_pl.json", "w"),
                indent=4,
            )
        ...
        for rgb_pl_idx in range(args.num_rgb_pls):
            ...
            json.dump(
                ...
                open(f"{ref_pl_path}/rgb_pl.json", "w"),
                indent=4,
            )
        ...
        for multi_pl_idx in range(args.num_multi_pls):
            pls = multi_pls[
                multi_pl_idx * args.max_pl_num : (multi_pl_idx + 1) * args.max_pl_num
            ]
            powers = [random.uniform(500, 1500) for _ in range(args.max_pl_num)]
            for pl_idx in range(args.max_pl_num):
                _point_light = create_point_light(
                    pls[pl_idx], powers[pl_idx], keep_other_lights=pl_idx > 0
                )
            ...
            json.dump(
                ...
                open(f"{ref_pl_path}/multi_pl.json", "w"),
                indent=4,
            )
        ...
        for env_idx in range(args.num_white_envs):
            ...
            strength = 1.0
            set_env_light(
                env_map_path, rotation_euler=rotation_euler, strength=strength
            )
            ...
            json.dump(
                ...
                open(f"{env_path}/white_env.json", "w"),
                indent=4,
            )
        ...
        for area_light_idx in range(args.num_area_lights):
            ...
            area_light_size = random.uniform(5.0, 10.0)
            _area_light = create_area_light(
                area_light_pos, area_light_power, area_light_size
            )
