...
class Options:
    ...
    cam_elev: float = 30.0  # Elevation of the camera in degrees
    cam_azi: float = 0.0  # Azimuth of the camera in degrees
    cam_dist: float = 1.5  # Distance of the camera from the origin
    cam_fov: float = 35.0  # Field of view of the camera
    pl_pos_r: float = 2.0  # Rotation radius of the point light
    pl_pos_center: List[float] = field(
        default_factory=lambda: [0.0, 0.0, 1.0]
    )  # Center position of the point light
    power: float = 100.0  # Power of the point light
...
def render_core(args: Options):
    ...
    if args.env_map is not None:
        ...
        for i in tqdm(range(args.frames), desc="Rendering Env Map Hints"):
            z_angle = (i / args.frames) * np.pi * 2.0
