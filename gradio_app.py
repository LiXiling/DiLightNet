...
with gr.Blocks(title="DiLightNet Demo") as demo:
    ...
    with gr.Row():
        ...
        with gr.Column(variant="panel"):
            ...
            input_image = gr.Image(
                height=512, width=512, label="Input Image", interactive=True
            )
            with gr.Accordion("Generate Image", open=False):
                with gr.Group():
                    ...
                    with gr.Row():
                        ...
                        down_from_768 = gr.Checkbox(
                            label="Downsample from 768", value=True
                        )
                with gr.Row():
                    generate_btn = gr.Button(value="Generate")
                    generate_btn.click(
                        fn=img_gen,
                        inputs=[prompt, seed, steps, cfg, down_from_768],
                        outputs=[input_image],
                    )
            gr.Examples(
                examples=[
                    os.path.join("examples/provisional_img", i)
                    for i in os.listdir("examples/provisional_img")
                ],
                inputs=[input_image],
                examples_per_page=8,
            )
        ...
        with gr.Column(variant="panel"):
            ...
            with gr.Tab("Masked Image"):
                masked_image = gr.Image(
                    height=512, width=512, label="Masked Image", interactive=True
                )
            with gr.Tab("Mask"):
            ...
            rm_bg_btn.click(
                fn=rm_bg, inputs=[input_image, use_sam], outputs=[masked_image, mask]
            )
        ...
        with gr.Column(variant="panel"):
            ...
            with gr.Column():
                with gr.Accordion("Options", open=False):
                    with gr.Group():
                        remove_edges = gr.Checkbox(
                            label="Remove Occlusion Edges", value=False
                        )
                        fov = gr.Number(value=55.0, label="FOV", interactive=False)
                ...
                def mesh_reconstruction_wrapper(
                    image,
                    mask,
                    remove_edges,
                    mask_threshold,
                    ...
                    return mesh_reconstruction(
                        image, mask, remove_edges, None, mask_threshold
                    )
    ...
    with gr.Row():
        with gr.Column(variant="panel"):
            ...
            is_env_lighting = gr.Checkbox(
                label="Use Environmental Lighting",
                value=True,
                interactive=False,
                visible=False,
            )
            with gr.Tab("Environmental Lighting"):
                env_map_preview = gr.Image(
                    label="Environment Map Preview",
                    height=256,
                    width=512,
                    interactive=False,
                    show_download_button=False,
                )
                env_map_path = gr.Text(
                    interactive=False, visible=False, value="examples/env_map/grace.exr"
                )
                env_rotation = gr.Slider(
                    value=0.0,
                ...
                env_examples = gr.Examples(
                    examples=[
                        [
                            os.path.join("examples/env_map_preview", i),
                            os.path.join("examples/env_map", i).replace("png", "exr"),
                        ]
                        for i in os.listdir("examples/env_map_preview")
                    ],
                ...
                def render_wrapper_env(
                    ...
                    res_path = render_hint_images_btn_func(
                        mesh,
                        float(fov),
                        [(0, 0, 0)],
                        env_map=env_map_path,
                        env_start_azi=env_rotation / 360.0,
                    )
                    hint_files = [
                        res_path + "/hint00" + mat
                        for mat in [
                    ...
                    for hint_file in hint_files:
                        ...
                        hints.append(hint)
                    hints = rearrange(
                        np.stack(hints), "(n1 n2) h w c -> (n1 h) (n2 w) c", n1=2, n2=2
                    )
            ...
            with gr.Tab("Point Lighting"):
                ...
                def render_wrapper_pl(
                    ...
                    res_path = render_hint_images_btn_func(
                        mesh, float(fov), [(pl_pos_x, pl_pos_y, pl_pos_z)], power
                    )
                    hint_files = [
                        res_path + "/hint00" + mat
                        for mat in [
        ...
        with gr.Column(variant="panel"):
            ...
            with gr.Group():
                relighting_prompt = gr.Textbox(
                    value="",
                    label="Appearance Text Prompt",
                    lines=3,
                    placeholder="Input prompt here",
                    interactive=True,
                )
                ...
                with gr.Row():
                    ...
                    very_specular_prompt_btn = gr.Button(
                        value="Very Specular", size="sm"
                    )
                metallic_prompt_btn.click(
                    fn=lambda x: x + " metallic",
                    inputs=[relighting_prompt],
                    outputs=[relighting_prompt],
                )
                specular_prompt_btn.click(
                    fn=lambda x: x + " specular",
                    inputs=[relighting_prompt],
                    outputs=[relighting_prompt],
                )
                very_specular_prompt_btn.click(
                    fn=lambda x: x + " very specular",
                    inputs=[relighting_prompt],
                    outputs=[relighting_prompt],
                )
                with gr.Row():
                    ...
                    reuse_btn = gr.Button(
                        value="Reuse Provisional Image Generation Prompt"
                    )
                ...
                reuse_btn.click(
                    fn=lambda x: x, inputs=[prompt], outputs=[relighting_prompt]
                )
            with gr.Accordion("Options", open=False):
            ...
            def gen_relighting_image(
                ...
                if do_env_inpainting:
                    bg = imageio.v3.imread(res_folder_path + f"/bg00.png") / 255.0
                else:
                    bg = np.zeros_like(relit_img)
                relit_img = relit_img / 255.0
                mask_for_bg = (
                    imageio.v3.imread(res_folder_path + "/hint00_diffuse.png")[..., -1:]
                    / 255.0
                )
            ...
            relighting_generate_btn.click(
                fn=gen_relighting_image,
                inputs=[
                    masked_image,
                    mask,
                    res_folder_path,
